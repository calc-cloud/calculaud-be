...
class TestHierarchiesAPI:
    ...
    def test_create_hierarchy_deep_nesting(self, test_client: TestClient):
        ...
        assert (
            level3_response.json()["path"] == "North Unit / Tech Center / Business ANAF"
        )
    ...
    def test_update_hierarchy_children_paths_updated(self, test_client: TestClient):
        ...
        assert (
            grandchild_response.json()["path"]
            == "Old Root / Child Unit / Grandchild Team"
        )
        ...
        child_get_response = test_client.get(
            f"{settings.api_v1_prefix}/hierarchies/{child_id}"
        )
        ...
        assert (
            grandchild_get_response.json()["path"]
            == "New Root / Child Unit / Grandchild Team"
        )
    ...
    def test_get_hierarchy_children_endpoint(self, test_client: TestClient):
        ...
        response = test_client.get(
            f"{settings.api_v1_prefix}/hierarchies/{root_id}/children"
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_hierarchy_search_functionality(self, test_client: TestClient):
        ...
        # Search for "software"
        response = test_client.get(
            f"{settings.api_v1_prefix}/hierarchies?search=software"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 1
