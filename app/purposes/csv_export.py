...
def extract_purchase_stage_data(purchase: Purchase) -> dict[str, str]:
    ...
    return {
        ...
        'emf_completion_date': (
            ...
            if emf_stage and emf_stage.completion_date else ""
        ),
    ...
    for purpose in purposes:
        ...
        for purchase in purpose.purchases:
            ...
            if any([
                ...
                bikushit_completion_dates.append(purchase_stage_data['bikushit_completion_date'])
                demand_ids.append(purchase_stage_data['demand_id'])
                demand_completion_dates.append(purchase_stage_data['demand_completion_date'])
                order_ids.append(purchase_stage_data['order_id'])
                order_completion_dates.append(purchase_stage_data['order_completion_date'])
...
def format_services_list(purpose_contents: list[PurposeContent]) -> str:
    ...
    return '\n'.join([
        ...
        for content in purpose_contents
    ])
...
def export_purposes_csv(db: Session, params: GetPurposesRequest) -> str:
    ...
    pending_stages_data = {
        purpose.id: '\n'.join([
            calculate_pending_stages_info(purchase) 
            for purchase in purpose.purchases
        ])
        for purpose in purposes
    }
