...
from fastapi import APIRouter, Depends, HTTPException, Query, status as statuses
from sqlalchemy.orm import Session

from app.database import get_db
from app.pagination import PaginationParams, PaginatedResult, create_paginated_result
from app.purposes import service
from app.purposes.models import StatusEnum
from app.purposes.schemas import Purpose, PurposeCreate
...
def get_purposes(
        ...
        search: str | None = Query(None, description="Search in description, content, and supplier"),
...
def patch_purpose(
        purpose_id: int,
        purpose_update: PurposeCreate,
        db: Session = Depends(get_db)
):
    """Patch an existing purpose."""
    patched_purpose = service.patch_purpose(
        db, purpose_id, purpose_update.model_dump(exclude_unset=True)
    )
